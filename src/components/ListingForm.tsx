...
import type { CategoryId, ConditionId, Handedness, Listing } from '@/types';
...
interface Props {
  /**
   * L'annonce à modifier. Absente, le formulaire publie.
   *
   * Un seul formulaire pour les deux : publier et modifier posent exactement
   * les mêmes questions, et les faire diverger garantirait qu'un champ ajouté
   * un jour ne le soit qu'd'un côté.
   */
  listing?: Listing;
  /** Après un enregistrement réussi, avec l'annonce telle qu'elle est en base. */
  onDone: (listing: Listing) => void;
}

/** Les champs techniques, remis sous la forme texte qu'attend le formulaire. */
function specsFromListing(listing: Listing): Record<string, string> {
  const valeurs: Record<string, string> = {};
  if (listing.drawWeight != null) valeurs.drawWeight = String(listing.drawWeight);
  if (listing.bowLength != null) valeurs.bowLength = String(listing.bowLength);
  if (listing.drawLength != null) valeurs.drawLength = String(listing.drawLength);
  if (listing.spine != null) valeurs.spine = String(listing.spine);
  if (listing.size) valeurs.size = listing.size;
  return valeurs;
}

/**
 * Les photos déjà en ligne, telles que le formulaire les manipule.
 *
 * `images` retombe sur la clé de la catégorie quand l'annonce n'a aucune
 * photo : cette clé n'est pas une photo, et la reprendre ici ferait croire au
 * vendeur qu'il en a une.
 */
function photosFromListing(listing: Listing): string[] {
  return listing.images.filter((image) => image.startsWith('http'));
}

export function ListingForm({ listing, onDone }: Props) {
  ...
  const [category, setCategory] = useState<CategoryId>(listing?.category ?? 'bow-recurve');
  const [brand, setBrand] = useState(listing?.brand ?? 'Hoyt');
  const [condition, setCondition] = useState<ConditionId>(listing?.condition ?? 'very-good');
  const [handedness, setHandedness] = useState<Handedness>(listing?.handedness ?? 'right');
  const [specs, setSpecs] = useState<Record<string, string>>(
    listing ? specsFromListing(listing) : {},
  );
  const [city, setCity] = useState(listing?.city ?? user?.city ?? '');
  const [shipping, setShipping] = useState(listing?.shipping ?? true);
  const [shippingPrice, setShippingPrice] = useState(
    listing?.shippingPrice != null ? String(listing.shippingPrice) : '',
  );
  const [parcelSize, setParcelSize] = useState<ParcelSize>(
    listing?.parcelSize ?? suggestedParcel(listing?.category ?? 'bow-recurve'),
  );
  const [photos, setPhotos] = useState<string[]>(listing ? photosFromListing(listing) : []);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (!user) return;
      // Un import remplit un formulaire vierge. En modification, il écraserait
      // l'annonce qu'on est venu corriger.
      if (edition) return;
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (listing) {
        // On ne vide rien : le vendeur reste devant son annonce, corrigée.
        onDone(await updateListing(listing.id, champs));
        return;
      }

      const publiee = await createListing(champs);
      setTitle('');
      setDescription('');
      setPrice('');
      setSpecs({});
      ...
      if (/rifiez votre identit/.test(message)) {
        ...
        Alert.alert(edition ? 'Enregistrement impossible' : 'Publication impossible', message);
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
      ...
      >
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <Button
            label={edition ? 'Enregistrer les modifications' : 'Publier l’annonce'}
